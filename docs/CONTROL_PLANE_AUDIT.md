...
| 2. Account/credential separation | `config/accounts.go`, `credential.go`, injected adapter resolution; Batch 63 blocks provider deletion while accounts remain bound | Finish provider-specific credential setup and migration UX |
| 3. Account model | Account configuration plus `/admin/status` account health aggregation: health/auth state, capacity, sources, counters, last success/failure and last validation; Batch 69 maps login-required and access-denied authentication evidence to `auth_required`; explicit account validation entry point; Batch 60 blocks deletion while sources remain bound | Provider-specific health evidence and broader account lifecycle |
