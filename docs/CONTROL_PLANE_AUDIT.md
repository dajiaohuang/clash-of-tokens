...
| 41. Administration API | Persistent CRUD, config, simulator, source checks, account validation, browser process controls and device doctor; Batch 60 returns a clear conflict for account deletion with bound sources; Batch 61 does the same for browser profiles | Broader browser/device lifecycle operations |
