...
with tempfile.TemporaryDirectory(prefix="cot-browser-test-") as profile_dir, sync_playwright() as playwright:
    ...
    # Provider, account and source Auto approval are independent switches.
    page.get_by_role("link", name="Accounts", exact=True).click()
    account_row = page.get_by_role("row").filter(has=page.get_by_text("UI test account", exact=True))
    account_row.get_by_role("button", name="Allow Auto", exact=True).click()
    expect(page.get_by_role("heading", name="Review changes", exact=True)).to_be_visible()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(account_row).to_contain_text("Approved")
    account_row.get_by_role("button", name="Remove Auto", exact=True).click()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(account_row).to_contain_text("Manual")
    account_row.get_by_role("button", name="Delete", exact=True).click()
    expect(page.get_by_role("heading", name="Delete account", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("Sources")
    page.get_by_role("button", name="Cancel", exact=True).click()
    page.get_by_role("link", name="Providers", exact=True).click()
    page.get_by_role("searchbox", name="Filter providers").fill("openai")
    provider_row = page.get_by_role("row").filter(has=page.get_by_role("button", name="openai", exact=True))
    expect(provider_row).to_contain_text("Official API")
    provider_row.get_by_role("button", name="Allow Auto", exact=True).click()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    ...
    page.get_by_role("button", name="openai", exact=True).click()
    expect(page.get_by_role("heading", name="openai", exact=True)).to_be_visible()
    page.get_by_role("button", name="Delete provider", exact=True).click()
    expect(page.get_by_role("heading", name="Delete provider", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("UI test account")
    page.get_by_role("button", name="Review deletion", exact=True).click()
    expect(page.get_by_role("alert")).to_contain_text("still has bound accounts or sources")
    page.get_by_role("button", name="Cancel", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    ...
    page.get_by_role("link", name="Accounts", exact=True).click()
    expect(page.get_by_role("cell", name="UI test account", exact=True)).to_be_visible()
    ...
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    expect(page.get_by_role("cell", name="ui-profile", exact=True).first).to_be_visible()
    ...
    page.get_by_role("link", name="Providers", exact=True).click()
    page.get_by_role("searchbox", name="Filter providers").fill("claude-web")
    page.get_by_role("button", name="claude-web", exact=True).click()
    expect(page.get_by_role("heading", name="claude-web", exact=True)).to_be_visible()
    page.get_by_role("button", name="Refresh session", exact=True).click()
    expect(page.get_by_role("heading", name="Login evidence", exact=True)).to_be_visible()
    page.get_by_role("button", name="Close", exact=True).click()
    page.get_by_role("link", name="Browsers", exact=True).click()
    profile_row = page.get_by_role("row").filter(has=page.get_by_text("ui-profile", exact=True)).first
    profile_row.get_by_role("button", name="Delete", exact=True).click()
    expect(page.get_by_role("heading", name="Delete browser profile", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("claude-account")
    page.get_by_role("button", name="Review deletion", exact=True).click()
    ...
    page.get_by_role("button", name="Add account", exact=True).click()
    page.get_by_label("Provider", exact=True).select_option("claude-web")
    page.get_by_label("ID", exact=True).fill("claude-import-account")
    page.get_by_label("Quota domain", exact=True).fill("claude-import-quota")
    page.get_by_role("button", name="Import password manager", exact=True).click()
    page.get_by_label("CSV or JSON file", exact=True).set_input_files({"name":"claude.csv","mimeType":"text/csv","buffer":b"name,url,username,password\nClaude,https://claude.ai/login,synthetic-claude-user,synthetic-claude-password\n"})
    ...
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    expect(page.get_by_role("row").filter(has=page.get_by_text("claude-import-account", exact=True))).to_contain_text("Manual")
    ...
    page.get_by_role("button", name="Add credential", exact=True).click()
    page.get_by_label("Credential ID", exact=True).fill("ui-cookie")
    page.get_by_label("Type", exact=True).select_option("cookie")
    page.get_by_label("Secret value", exact=True).fill("synthetic-cookie-not-for-display")
    page.get_by_role("button", name="Save credential", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    expect(page.get_by_text("cred://ui-cookie", exact=True)).to_be_visible()
    assert "synthetic-cookie-not-for-display" not in page.content(), "Cookie echoed into DOM"
    ...
    page.get_by_role("button", name="Import selected", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    expect(page.get_by_text("selected-export", exact=True)).to_have_count(3)
    page.get_by_role("link", name="Accounts", exact=True).click()
    ...
    expect(page.get_by_role("heading", name="Accounts", exact=True)).to_be_visible()
    # An incompatible protected reference is available only after the account
    # wizard's explicit reviewed override is enabled.
    page.get_by_role("button", name="Add account", exact=True).click()
    wizard = page.get_by_role("dialog")
    wizard.get_by_label("Provider", exact=True).select_option("openai")
    wizard.get_by_label("Reviewed credential type override", exact=True).check()
    wizard.get_by_label("Credential", exact=True).select_option("cred://ui-cookie")
    wizard.get_by_label("ID", exact=True).fill("override-account")
    wizard.get_by_label("Quota domain", exact=True).fill("override-quota")
    wizard.get_by_role("button", name="Review changes", exact=True).click()
    expect(page.get_by_role("heading", name="Review changes", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("credential_type_override")
    ...
    expect(page.get_by_role("dialog")).not_to_be_visible()
    page.get_by_role("link", name="Accounts", exact=True).click()
    bound_account_row = page.get_by_role("row").filter(has=page.get_by_text("UI test account", exact=True))
    bound_account_row.get_by_role("button", name="Delete", exact=True).click()
    expect(page.get_by_role("heading", name="Delete account", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("openai")
    page.get_by_role("button", name="Review deletion", exact=True).click()
    expect(page.get_by_role("alert")).to_contain_text("still has bound sources")
    page.get_by_role("button", name="Cancel", exact=True).click()
    page.get_by_role("link", name="Sources", exact=True).click()
    expect(page.get_by_role("heading", name="Sources", exact=True)).to_be_visible()
    source_row = page.get_by_role("row").filter(has=page.get_by_role("button", name="openai", exact=True))
    source_row.get_by_role("button", name="Edit", exact=True).click()
    ...
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(source_row.get_by_role("button", name="Remove Auto", exact=True)).to_be_visible()
    source_row.get_by_role("button", name="Remove Auto", exact=True).click()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    # Model enablement is an independent routing gate. Exercise the bulk
    # editor and restore the configured model before continuing.
    page.get_by_role("link", name="Models", exact=True).click()
    model_row = page.get_by_role("row").filter(has=page.get_by_text("test-model", exact=True))
    model_row.get_by_role("checkbox", name="Select model openai/test-model", exact=True).check()
    page.get_by_role("button", name="Edit selected models", exact=True).click()
    page.get_by_label("Models enabled", exact=True).select_option(label="No")
    page.get_by_role("button", name="Review model changes", exact=True).click()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(page.get_by_role("row").filter(has=page.get_by_text("test-model", exact=True))).to_contain_text("Disabled")
    model_row = page.get_by_role("row").filter(has=page.get_by_text("test-model", exact=True))
    model_row.get_by_role("checkbox", name="Select model openai/test-model", exact=True).check()
    page.get_by_role("button", name="Edit selected models", exact=True).click()
    page.get_by_label("Models enabled", exact=True).select_option(label="Yes")
    page.get_by_role("button", name="Review model changes", exact=True).click()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(page.get_by_role("row").filter(has=page.get_by_text("test-model", exact=True))).to_contain_text("Enabled")
    page.get_by_role("link", name="Sources", exact=True).click()
    ...
    page.get_by_role("link", name="Providers", exact=True).click()
    page.get_by_role("searchbox", name="Filter providers").fill("openai")
    provider_list_row = page.get_by_role("row").filter(has=page.get_by_role("button", name="openai", exact=True))
    expect(provider_list_row.get_by_role("button", name="Validate", exact=True)).to_be_visible()
    page.get_by_role("button", name="openai", exact=True).click()
    ...
    page.get_by_label("API key", exact=True).fill("replacement-synthetic-secret")
    page.get_by_role("button", name="Save credential", exact=True).click()
    ...
    expect(page.get_by_text("explicit_stream_generation", exact=True)).to_have_count(5)
    ...
    expect(page.get_by_role("cell", name="chat / Validation", exact=True)).to_have_count(5)
    ...
    wizard = page.get_by_role("dialog")
    page.get_by_label("Provider", exact=True).select_option("openai")
    expect(wizard.get_by_role("button", name="Import token", exact=True)).to_be_visible()
    expect(wizard.get_by_role("button", name="Import password manager", exact=True)).to_have_count(0)
    expect(wizard.get_by_role("button", name="Import browser cookies", exact=True)).to_have_count(0)
    expect(wizard.get_by_role("button", name="New isolated profile", exact=True)).to_have_count(0)
    expect(wizard.get_by_role("button", name="Login with selected profile", exact=True)).to_have_count(0)
    page.get_by_label("Provider", exact=True).select_option("doubao")
    expect(wizard.get_by_role("button", name="Import token", exact=True)).to_have_count(0)
    expect(wizard.get_by_role("button", name="Import browser cookies", exact=True)).to_be_visible()
    expect(wizard.get_by_role("button", name="New isolated profile", exact=True)).to_be_visible()
    expect(wizard.get_by_role("button", name="Login with selected profile", exact=True)).to_be_visible()
    page.get_by_label("Provider", exact=True).select_option("openai")
    wizard.get_by_role("button", name="New credential", exact=True).click()
    expect(page.get_by_role("heading", name="Add credential", exact=True)).to_be_visible()
    expect(page.get_by_label("API key", exact=True)).to_be_visible()
    expect(page.get_by_label("Username", exact=True)).to_be_hidden()
    page.get_by_role("button", name="Close", exact=True).click()
    page.get_by_role("button", name="Add account", exact=True).click()
    wizard = page.get_by_role("dialog")
    page.get_by_label("Provider", exact=True).select_option("claude-web")
    wizard.get_by_role("button", name="New credential", exact=True).click()
    page.get_by_label("Type", exact=True).select_option("username_password")
    expect(page.get_by_label("Username", exact=True)).to_be_visible()
    expect(page.get_by_label("Password", exact=True)).to_be_visible()
    page.get_by_role("button", name="Close", exact=True).click()
    page.get_by_role("button", name="Add account", exact=True).click()
    wizard = page.get_by_role("dialog")
    page.get_by_label("Provider", exact=True).select_option("codex")
    wizard.get_by_role("button", name="New credential", exact=True).click()
    credential_dialog = page.get_by_role("dialog")
    credential_types = credential_dialog.get_by_label("Type", exact=True)
    expect(credential_types.locator("option[value='oauth']")).to_have_count(1)
    expect(credential_types.locator("option[value='cli_session']")).to_have_count(1)
    expect(credential_types.locator("option[value='api_key']")).to_have_count(1)
    expect(credential_types.locator("option[value='cookie']")).to_have_count(0)
    credential_dialog.get_by_role("button", name="Close", exact=True).click()
    page.get_by_role("button", name="Add account", exact=True).click()
    wizard = page.get_by_role("dialog")
    page.get_by_label("Provider", exact=True).select_option("openai")
    ...
    page.get_by_role("button", name="Import token", exact=True).click()
    page.get_by_label("Import from", exact=True).select_option("environment")
    page.get_by_label("Configured source for environment import", exact=True).select_option("openai")
    page.get_by_role("button", name="Preview token", exact=True).click()
    page.get_by_role("button", name="Save imported token", exact=True).click()
    expect(page.get_by_role("heading", name="Set up account", exact=True)).to_be_visible()
    ...
    expect(page.get_by_role("dialog")).not_to_be_visible()
    page.get_by_role("link", name="Sources", exact=True).click()
    bulk_source_row = page.get_by_role("row").filter(has=page.get_by_text("bulk-source", exact=True))
    bulk_source_row.get_by_role("button", name="Edit", exact=True).click()
    page.get_by_label("Quota domain", exact=True).fill("ui-quota-source-review")
    page.get_by_role("button", name="Review changes", exact=True).click()
    expect(page.get_by_role("heading", name="Review quota change", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("UI test account")
    expect(page.get_by_role("dialog")).to_contain_text("bulk-source")
    page.get_by_role("button", name="Back", exact=True).click()
    expect(page.get_by_role("heading", name="Edit Source", exact=False)).to_be_visible()
    page.get_by_role("button", name="Cancel", exact=True).click()
    bulk_source_row.get_by_role("button", name="Delete", exact=True).click()
    expect(page.get_by_role("heading", name="Delete source", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("auto")
    page.get_by_role("button", name="Cancel", exact=True).click()
    ...
    page.get_by_role("button", name="Review changes", exact=True).click()
    expect(page.get_by_role("heading", name="Review changes", exact=True)).to_be_visible()
    expect(page.get_by_text("Restart required for: runtime.", exact=False)).to_be_visible()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    ...
    page.get_by_role("link", name="Health", exact=True).click()
    page.get_by_role("button", name="Edit shared quota", exact=True).click()
    expect(page.get_by_role("heading", name="Edit shared quota", exact=True)).to_be_visible()
    expect(page.get_by_role("dialog")).to_contain_text("UI test account")
    expect(page.get_by_role("dialog")).to_contain_text("openai")
    page.get_by_role("button", name="Close", exact=True).click()
    expect(page.get_by_role("dialog")).not_to_be_visible()
    ...
    page.get_by_role("link", name="Credentials", exact=True).click()
    credential_row = page.get_by_role("row").filter(has=page.get_by_text("cred://ui-token", exact=True))
    credential_row.get_by_role("button", name="Replace", exact=True).click()
    expect(page.get_by_role("dialog")).to_contain_text("API key")
    page.get_by_role("button", name="Close", exact=True).click()
    credential_row.get_by_role("button", name="Unbind", exact=True).click()
    expect(page.get_by_role("heading", name="Unbind credential", exact=True)).to_be_visible()
    page.get_by_role("button", name="Review unbind", exact=True).click()
    expect(page.get_by_role("heading", name="Review changes", exact=True)).to_be_visible()
    page.get_by_role("button", name="Apply changes", exact=True).click()
    credential_rows = page.get_by_role("row").filter(has=page.get_by_text("cred://ui-token", exact=True))
    expect(credential_rows.first).to_contain_text("Unbound")
    assert not errors, errors
    browser.close()
