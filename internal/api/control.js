...
async function providerDetail(provider){
 ...
 dialog(provider.id,[
  ...
  h('h3',{},'Accounts'),table(['Account','Enabled / Auto','Credential','Credential state','Browser authentication','In flight / limit','Actions'],accounts.map(a=>{
   ...
   return [a.display_name||a.id,(a.enabled?'Yes':'No')+' / '+(a.auto_approved?'Yes':'No'),a.credential_ref||'Not bound',credentialState+credentialVersion,accountAuth(a),(capacity?.active||0)+' / '+a.max_inflight,[button('Edit account',()=>edit('accounts',a)),a.browser_profile_id?button('Check login',()=>loginEvidence(a)):null]];
...
function accounts(){
 ...
 return [pageHead('Accounts','Account switches and capacity apply across their sources.',button('Discover candidates',discoverAccounts),button('Account pools',accountPools),button('Refresh capacity',refresh),button('Add account',()=>addAccount(),'primary')),table(['Account','Provider','Health','Auth status','Browser authentication','Credential','Credential state','Quota / in flight','Auto','Actions'],(S.config.accounts||[]).map(a=>[
  a.display_name||a.id,a.provider_id,badge(healthFor(a).health||'untested',healthFor(a).health==='healthy'?'good':healthFor(a).health==='disabled'?'':'warn'),healthFor(a).auth_status||'not_checked',accountAuth(a),a.credential_ref||'Not bound',(healthFor(a).credential_state||'not_configured')+(healthFor(a).credential_version?' · v'+healthFor(a).credential_version:''),a.quota_domain+' · '+((S.status.accounts||[]).find(x=>x.id===a.id)?.active||0)+' / '+a.max_inflight,badge(a.auto_approved?'Approved':'Manual',a.auto_approved?'accent':''),
...
async function sourceDetail(source){
 ...
 dialog('Source / '+source.id,[
  h('h3',{},'Configuration'),table(['Setting','Value'],[['Provider / adapter',source.provider+' / '+source.adapter],['Base URL',source.base_url||'Not configured'],['Organization',source.organization||'Not configured'],['Project',source.project||'Not configured'],['Account',account?.display_name||source.account_id||'No account'],['Credential configuration',verification?.credential_state||'Not established'],['Credential mode',source.credential_mode||'Not declared'],['Source type',source.source_kind||'Unspecified'],['Execution location',source.execution_location||'Not declared'],['Inference location',source.inference_location||'Not declared'],['Billing mode',source.billing_mode||'Not declared'],['Source enabled',source.enabled?'Yes':'No'],['Source Auto approval',source.auto_approved?'Yes':'No'],['Runtime routing state',state(source)],['Source concurrency',(runtime.active||0)+' / '+source.max_inflight],['Account concurrency',pool?pool.active+' / '+pool.limit:'No account limit'],['Shared quota',source.quota_domain+' · '+(quota?quota.active+' / '+quota.limit:'Not observed')],['Groups',S.config.groups.filter(g=>g.sources.includes(source.id)).map(g=>g.id).join(', ')||'None']]),
...
function health(){
 ...
 return [pageHead('Health','Recorded outcomes, cooldowns and account capacity.'),h('h2',{},'Provider health'),table(['Provider','Enabled','Auto','Pool strategy','Catalog','Verified sources','Health','Auth','In flight / limit','Accounts','Sources','Success / failure','Last validation','Last auth check'],providerRows,'No providers configured.'),h('h2',{},'Account health'),table(['Account','Provider','Enabled','Auto','Pool strategy','Weight','Credential state','Health','Auth','In flight / limit','Sources','Success / failure','Last validated','Last auth check'],accountRows,'No accounts configured.'),h('h2',{},'Source health'),table(['Source','Account','State','Credential','In flight','Success / failure','Average duration','Cooldown until','Last validation'],S.config.sources.map(source=>{
...
function searchResults(query){
 ...
 for(const kind of ['accounts','sources','groups'])for(const item of S.config[kind]||[])if([item.id,item.display_name,item.provider_id,item.provider,item.account_id,item.quota_domain].filter(Boolean).join(' ').toLowerCase().includes(q))results.push([item.display_name||item.id,title(kind),()=>kind==='sources'?sourceDetail(item):edit(kind,item)]);
