...
async function providerDetail(provider){
 ...
 dialog(provider.id,[
  ...
  h('h3',{},'Configured runtime'),table(['Measure','Value'],[['Provider enabled',configured?(configured.enabled?'Yes':'No'):'Not configured'],['Provider Auto approval',configured?(configured.auto_approved?'Yes':'No'):'Not configured'],['Pool strategy',providerHealth.pool_strategy||configured?.pool_strategy||'round-robin'],['Provider health',providerHealth.health||'untested'],['Provider auth status',providerHealth.auth_status||'not_checked'],['Catalog implementation',providerHealth.catalog_implemented?(providerHealth.catalog_live_verified?'Live catalog':'Implemented catalog'):'Not implemented'],['Verified sources',String(providerHealth.verified_sources||0)+' / '+sources.length],['Last generation validation',providerHealth.last_validated_at?new Date(providerHealth.last_validated_at).toLocaleString():'Not checked'],['Last auth check',providerHealth.last_auth_checked_at?new Date(providerHealth.last_auth_checked_at).toLocaleString():'Not checked'],['Accounts / sources / models',accounts.length+' / '+sources.length+' / '+sources.reduce((n,s)=>n+s.models.length,0)],['Sources with matching generation evidence',verified.size+' / '+sources.length],['Successful / failed requests',completed+' / '+failed],['Observed success rate',completed+failed?(completed*100/(completed+failed)).toFixed(1)+'%':'No completed requests']]),
...
function health(){
 const accountRows=(S.config.accounts||[]).map(account=>{const row=(S.status.account_health||[]).find(x=>x.id===account.id)||{};return [account.id,account.provider_id,row.pool_strategy||'round-robin',row.weight||account.weight||'—',badge(row.health||'untested',row.health==='healthy'?'good':row.health==='disabled'?'':'warn'),row.auth_status||'not_checked',(row.active||0)+' / '+(row.limit||account.max_inflight),row.sources?.join(', ')||'None',(row.completed||0)+' / '+(row.failures||0),row.last_validated_at?new Date(row.last_validated_at).toLocaleString():'Not checked',row.last_auth_checked_at?new Date(row.last_auth_checked_at).toLocaleString():'Not checked']});
 const providerRows=(S.status.provider_health||[]).map(row=>[row.id,row.enabled?'Yes':'No',row.pool_strategy||'round-robin',row.catalog_implemented?(row.catalog_live_verified?'Live catalog':'Implemented catalog'):'Not implemented',row.verified_sources||0,badge(row.health||'untested',row.health==='healthy'?'good':row.health==='disabled'?'':'warn'),row.auth_status||'not_checked',(row.active||0)+' / '+(row.limit||'—'),row.accounts?.join(', ')||'None',row.sources?.join(', ')||'None',(row.completed||0)+' / '+(row.failures||0),row.last_validated_at?new Date(row.last_validated_at).toLocaleString():'Not checked',row.last_auth_checked_at?new Date(row.last_auth_checked_at).toLocaleString():'Not checked']);
 return [pageHead('Health','Recorded outcomes, cooldowns and account capacity.'),h('h2',{},'Provider health'),table(['Provider','Enabled','Pool strategy','Catalog','Verified sources','Health','Auth','In flight / limit','Accounts','Sources','Success / failure','Last validation','Last auth check'],providerRows,'No providers configured.'),h('h2',{},'Account health'),table(['Account','Provider','Pool strategy','Weight','Health','Auth','In flight / limit','Sources','Success / failure','Last validated','Last auth check'],accountRows,'No accounts configured.'),h('h2',{},'Source health'),table(['Source','State','In flight','Success / failure','Average duration','Cooldown until'],S.config.sources.map(source=>{
