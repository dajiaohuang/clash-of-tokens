...
function health(){
 const accountRows=(S.config.accounts||[]).map(account=>{const row=(S.status.account_health||[]).find(x=>x.id===account.id)||{};return [account.id,account.provider_id,badge(row.health||'untested',row.health==='healthy'?'good':row.health==='disabled'?'':'warn'),row.auth_status||'not_checked',(row.active||0)+' / '+(row.limit||account.max_inflight),row.sources?.join(', ')||'None',(row.completed||0)+' / '+(row.failures||0),row.last_validated_at?new Date(row.last_validated_at).toLocaleString():'Not checked']});
 const providerRows=(S.status.provider_health||[]).map(row=>[row.id,row.enabled?'Yes':'No',badge(row.health||'untested',row.health==='healthy'?'good':row.health==='disabled'?'':'warn'),row.auth_status||'not_checked',(row.active||0)+' / '+(row.limit||'—'),row.accounts?.join(', ')||'None',row.sources?.join(', ')||'None',(row.completed||0)+' / '+(row.failures||0)]);
 return [pageHead('Health','Recorded outcomes, cooldowns and account capacity.'),h('h2',{},'Provider health'),table(['Provider','Enabled','Health','Auth','In flight / limit','Accounts','Sources','Success / failure'],providerRows,'No providers configured.'),h('h2',{},'Account health'),table(['Account','Provider','Health','Auth','In flight / limit','Sources','Success / failure','Last validated'],accountRows,'No accounts configured.'),h('h2',{},'Source health'),table(['Source','State','In flight','Success / failure','Average duration','Cooldown until'],S.config.sources.map(source=>{
