...
function validateSource(source){
 ...
 dialog('Validate source',[field('Model to validate',model),field('Protocol to validate',proto),h('p',{class:'muted'},'Sends one generation request: “Reply with OK.” Provider usage may be billed. This can check a disabled source without enabling its routes. No automatic retry is performed.')],[button('Run validation',async()=>{
  ...
  await refresh();
  dialog('Validation evidence',h('div',{},h('p',{class:'muted'},'One explicit request; no automatic retry.'),table(['Check','Result'],[['Source / model',result.source+' / '+result.model],['Verified',result.verified?'Yes':'No'],['Checked at',result.checked_at],['History saved',result.history_recorded?'Yes':'No: result was not persisted'],['Connection',result.checks?.connection||'Not reported'],['Authentication',result.checks?.auth||'Not reported'],['Request',result.checks?.request||'Not reported'],['Streaming',result.checks?.streaming||'Not reported'],['Completion',result.checks?.completion||'Not reported'],['Latency',result.checks?.duration_ms===undefined?'Not reported':Number(result.checks.duration_ms).toFixed(1)+' ms'],['Output observed',result.output_observed?'Yes':'No'],['Error',result.result.upstream_error||'None']]));
 },'primary')]);
}
function models(){
