...
func TestSessionMetadataUsesHashedIDsAndSupportsLifecycle(t *testing.T) {
	c := New(config.Source{ID: "yuanbao-source", Adapter: AdapterYuanbao})
	state, err := c.session("private-session-key")
	if err != nil {
		t.Fatal(err)
	}
	state.touch("yuanbao", "chat")
	state.mu.Lock()
	state.conversationID = "provider-conversation"
	state.mu.Unlock()
	...
	if items[0].Conversation != "provider-conversation" || items[0].Source != "yuanbao-source" {
